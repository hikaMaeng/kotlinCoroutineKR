...
## 사용 분야
코루틴은 suspendable computation의 인스턴스라 할 수 있다. 따라서 특정 지점에서 일시정지한 코루틴은 나중에 다른 쓰레드에서 재게될 수 있다. 코루틴끼리는 서로가 데이터를 주고 받으며 호출할 수 있으므로 협업 멀티태스킹을 위한 매커니즘을 구현할 수 있다.
**역주**
suspendable computation이란 프로그램이 특정 위치에서 일시중지(suspend)할 수 있고 나중에 다른 쓰레드에서 실행을 재개할 수 있는 것을 말합니다.
